...
/**
 * Created by limaoi on 2017/12/17.
...
public class VideoPicAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private List<AlbumFile> mPicAlbumFiles;

    public VideoPicAdapter(Context context, int itemSize, OnItemClickListener itemClickListener) {
    ...
    public void notifyDataSetChanged(List<AlbumFile> imagePathList) {
        this.mPicAlbumFiles = imagePathList;
        super.notifyDataSetChanged();
    }

    @Override
    public int getItemViewType(int position) {
        AlbumFile albumFile = mPicAlbumFiles.get(position);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        switch (viewType) {
            case AlbumFile.TYPE_IMAGE: {
                ((ImageViewHolder) holder).setData(mPicAlbumFiles.get(position));
                break;
            }
    ...
    public int getItemCount() {
        return mPicAlbumFiles == null ? 0 : mPicAlbumFiles.size();
    }
